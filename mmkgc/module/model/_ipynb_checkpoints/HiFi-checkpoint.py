...
class HiFi(Model):
    def __init__(
        ...
        super(HiFi, self).__init__(ent_tot, rel_tot)
        ...
        self.dim_e = dim * 2
        self.dim_r = dim
        self.lambda_final = lambda_final  # Store lambda_final parameter
        ...
        self.st_attention_linear = nn.Linear(self.dim_e * 2, 2)  # Initialize structure-text attention linear layer
        self.si_attention_linear = nn.Linear(self.dim_e * 2, 2)  # Initialize structure-image attention linear layer

        self.attention_linear = nn.Linear(self.dim_e * 3, 4)
        self.attention_linear.requires_grad_(True)
    ...
    def get_joint_embeddings(self, es, ev, et):
        st_att_scores = torch.sigmoid(self.st_attention_linear(torch.cat([es, et], dim=-1)))
        si_att_scores = torch.sigmoid(self.si_attention_linear(torch.cat([es, ev], dim=-1)))
        hts_eh = st_att_scores[:, 0].unsqueeze(-1) * es + st_att_scores[:, 1].unsqueeze(-1) * et
        hvs_eh = si_att_scores[:, 0].unsqueeze(-1) * es + si_att_scores[:, 1].unsqueeze(-1) * ev
        final_joint_embeddings = self.lambda_final * hts_eh + (1 - self.lambda_final) * hvs_eh
        return final_joint_embeddings
    ...
    def forward(self, data):
        ...
        h_joint = self.get_joint_embeddings(h_text_emb, h_img_emb, h)
        t_joint = self.get_joint_embeddings(t_text_emb, t_img_emb, t)
