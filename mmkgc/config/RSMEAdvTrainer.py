...
# 以上代码定义了一个新的对抗训练器类 RSMEAdvTrainer，用于训练模型并生成对抗样本。该类的设计与之前的对抗训练器类相似，但是专门用于生成对抗样本。
class RSMEAdvTrainer(object):
    ...
    def train_one_step(self, data):
        ...
        scores = self.model.model.get_fake_score(
            batch_h=batch_h_gen,
            batch_r=batch_r,
            batch_t=batch_t_gen,
            mode=data['mode'],
        ...
        for score in scores:
            loss += self.model.loss(p_score, score)
        loss.backward()
        self.optimizer.step()
        # training G
